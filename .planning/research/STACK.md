# Technology Stack Research: LineClear

**Domain:** Local-first Ruby/Rails verification gem and CLI
...
- **Recommendation — MEDIUM:** an implementation choice derived from the verified facts and LineClear's stated product constraints.
- Versions in this document are the versions observed on 2026-08-16, not unbounded promises of future compatibility.

## Executive Recommendation

**Recommendation — MEDIUM:** Ship LineClear as a conventional Ruby gem with a zero-framework core, `lineclear` executable, Bundler/RubyGems packaging, Rake tasks, Minitest for the project's own tests, and Markdown plus RDoc documentation. Keep every target-project analyzer external: detect it through the target bundle, execute its native command, record its exact version, and translate its output into LineClear's canonical schema.

Set `required_ruby_version` to `>= 3.3`. Test the core on Ruby 3.3, 3.4, and 4.0. Treat Rails 8.0 as the minimum supported Rails context and test Rails 8.0 and 8.1 without declaring Rails or ActiveSupport as runtime dependencies. This omits already-EOL Ruby 3.2 and follows maintained upstream series instead of accumulating compatibility shims.

The first analyzer tranche should be Minitest, RSpec, RuboCop with optional `rubocop-rails`, SimpleCov, and then `bundler-audit`. Defer runtime-instrumented, overlapping, license-sensitive, or weakly structured integrations until their contracts are researched with fixtures.

## Supported Ruby and Rails Policy

### Proposed Baseline

| Surface | Verified upstream state | LineClear policy | Confidence |
|---|---|---|---|
...
“Rails support” means that Rails-aware discovery, diff classification, and adapters are tested in representative Rails applications. It does not justify adding `rails`, `railties`, or `activesupport` to the gemspec.

### CI Matrix

**Recommendation — MEDIUM:** Keep the matrix deliberate rather than Cartesian.
...
Use separate appraisal Gemfiles only when Rails-dependent test fixtures exist. Until then, do not add Appraisal merely to model a matrix that contains no Rails API usage.
...
| Minitest | 6.0.6 for LineClear's suite | Unit/integration testing | MIT licensed, Ruby >= 3.2, small, and adequate for a gem. Target-project Minitest compatibility remains `>= 5, < 7`. [RubyGems: Minitest](https://rubygems.org/gems/minitest) | Verified fact / recommendation — MEDIUM |
...
|---|---|---|---|---|
| `json_schemer` | `>= 2.5, < 3` | JSON Schema 2020-12 validation | Add as the single intentional third-party runtime dependency when LineClear validates its canonical report before writing it. Version 2.5.0 is MIT licensed, supports JSON Schema 2020-12, and requires Ruby >= 2.7. [RubyGems: json_schemer](https://rubygems.org/gems/json_schemer), [official repository](https://github.com/davishmcclurg/json_schemer) | Verified fact / recommendation — MEDIUM |

Do not add `json_schemer` before the validation path lands. Apart from that schema validator, the initial runtime dependency count should be zero.
...
**Recommendation — MEDIUM:** Development-only dependencies belong in the `Gemfile`, not as gemspec runtime dependencies. Commit the lockfile used by CI. Keep the gemspec deterministic: explicit file list from Git, `required_ruby_version`, Apache-2.0 license, source/changelog/documentation URIs, and MFA metadata.

```ruby
# lineclear.gemspec (policy excerpt)
...
All entries below are **external target-project tools**. LineClear should not vendor, repackage, or silently install them. “Commercial use” is a practical license-screening note, not legal advice.
...
| Undercover | [Official repository](https://github.com/grodowski/undercover), [RubyGems 0.8.5](https://rubygems.org/gems/undercover) | MIT (`MIT`); permissive commercial use with notice obligations | `bundle add --group test undercover --require false` | External; never a LineClear runtime dependency | If later adopted: `>= 0.8, < 1` with syntax-version and output fixtures | `--format json` exists, but no versioned public schema was found; it consumes SimpleCov data and overlaps LineClear's changed-line coverage calculation. | Verified fact / recommendation — MEDIUM |
...
|---:|---|---|---|---|
| 1 | Minitest + RSpec test results | Test pass/fail and failure details are table stakes. Both are common Rails test frameworks and add no analyzer runtime to LineClear. | Own the Minitest reporter format; translate RSpec's documented JSON with version fixtures. | Recommendation — MEDIUM |
...
| 3 | SimpleCov | Provides the stable raw coverage artifact needed for LineClear's “changed executable line” gate. Its public `coverage.json` schema is substantially safer than parsing internal state. | Accept schema v1 `coverage.json`; compute diff-line coverage inside LineClear; never parse `.resultset.json`. | Recommendation — MEDIUM |
...
|---|---|---|---|---|
| Undercover | Defer | Duplicates SimpleCov plus git-diff logic already central to LineClear; current syntax choices documented by the tool lag the newest Ruby lines. | Demonstrate unique detection value, Ruby 3.4/4.0 syntax coverage, and a fixture-stable JSON contract. | Recommendation — MEDIUM |
...
| Prosopite | Defer to runtime-evidence phase | Requires application boot, database activity, and SQL event instrumentation; lacks a native stable machine format. | Safe test command, isolation guarantees, timeout behavior, and a LineClear-owned logger protocol. | Recommendation — MEDIUM |
...
**Recommendation — MEDIUM:** An analyzer being installed is not enough to call it supported. Every adapter must define:

1. A closed supported version range and fixture outputs from its lower/current major boundaries.
2. The exact command as an argument array, working directory, target bundle, environment allowlist, timeout, and process-tree shutdown behavior.
3. Exit-code semantics separated from parser success. A parser or timeout failure must be “unavailable/error,” never converted into a clean result.
4. Tool version, plugin versions, configuration digest, command scope, and artifact digest in the evidence envelope.
5. A forward-compatible parser that ignores unknown fields but rejects missing required identity/result fields.
6. A public machine format when available. Human text parsing requires a phase-specific research flag and fixture corpus.

Do not pin analyzer gems in LineClear's gemspec. Resolve them through the target project's `bundle exec`, because the target lockfile and configuration are part of the evidence.

## Publishing and Supply Chain

### Recommended Release Path

**Verified fact — MEDIUM:** RubyGems Trusted Publishing exchanges a GitHub Actions OIDC identity for a short-lived, gem-scoped token, avoiding a stored long-lived RubyGems API key. RubyGems provides the official `rubygems/release-gem` action/workflow for this path. [Trusted Publishing guide](https://guides.rubygems.org/trusted-publishing/), [official release action](https://github.com/rubygems/release-gem)

**Recommendation — MEDIUM:**

1. Configure RubyGems Trusted Publishing for the exact GitHub owner, repository, workflow filename, and protected `release` environment.
2. Trigger publishing only from a protected version tag after ordinary CI succeeds. Require environment approval for the release job.
3. Give the release job only the permissions required by the official workflow: `id-token: write` for OIDC, `contents: read` normally, and `contents: write` only if that chosen workflow must create a tag or GitHub release.
4. Pin third-party actions in the release workflow to reviewed full commit SHAs and update them through reviewed dependency PRs. GitHub identifies a full-length commit SHA as the immutable action reference. [GitHub secure-use guidance](https://docs.github.com/en/actions/reference/security/secure-use)
5. Build from a clean tag, validate the gemspec, build the `.gem`, install that artifact into a clean environment, and smoke-test `lineclear --version` and schema validation before publication.
6. Publish the exact tested artifact through trusted publishing, verify the RubyGems checksum and provenance entry, then create the GitHub release from the same tag. Do not rebuild independently for the GitHub release.
7. Set `spec.metadata["rubygems_mfa_required"] = "true"`; RubyGems documents this opt-in metadata for MFA-protected gem operations. [MFA metadata guide](https://guides.rubygems.org/mfa-requirement-opt-in/)
...
GitHub artifact attestations are useful if LineClear later distributes separate binaries or release assets. They are not required for the initial gem-only release because RubyGems Trusted Publishing already supplies provenance; avoid maintaining two overlapping provenance systems without a consumer need. [GitHub artifact attestations](https://docs.github.com/en/actions/how-tos/secure-your-work/use-artifact-attestations/use-artifact-attestations)
...
| Framework integration | Rails-free core and optional fixtures | Rails/ActiveSupport runtime dependency | LineClear analyzes repositories; it does not need to become a Rails engine. |
...
| Analyzer installation | Target bundle, external processes | Vendored analyzers or a LineClear analyzer bundle | Avoids license coupling, version conflicts, large install footprint, and hidden behavior. |
| Coverage | SimpleCov public `coverage.json` + LineClear diff logic | SimpleCov `.resultset.json` or Undercover in Phase 2 | The public schema is versioned; internal result storage is not. |
...
gem build lineclear.gemspec
gem install --install-dir tmp/gem-home ./lineclear-*.gem
```

The final release task should use an explicit artifact path rather than a wildcard; the wildcard above is illustrative local setup, not the release workflow contract.
...
All sources were accessed on **2026-08-16**.
