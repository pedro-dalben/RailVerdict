...
| T-15 | Spoofing | Dated registry results, qualified legal conclusions, implementation evidence, or ASVS use is overstated as name clearance, deployed controls, or certification. | MEDIUM | Transfer | Preserve dates, limitations, non-blocking recommendation status, and evidence owner; recommend qualified review before publication (non-blocking by maintainer decision 2026-08-19; residual risk accepted); describe ASVS only as a control catalog. | Engineering checks cannot determine confusing similarity, jurisdictional rights, or certify a non-web CLI against ASVS. | Named reviewer, jurisdiction, scope, evidence, date, and conclusion; implemented controls retain phase-specific proof. | Phase 9 publication revalidation; qualified legal reviewer (recommended, non-blocking by maintainer decision). |
...
| C-10 Honest assurance | Draft, implemented, verified, certified, legally reviewed (recommended, non-blocking by maintainer decision 2026-08-19), and not applicable remain distinct states with named evidence; preliminary screen is not certification. | Every phase; Phase 9 release review. |
...
Trademark/name qualified review is NOT PERFORMED — NON-BLOCKING BY MAINTAINER DECISION 2026-08-19 (Pedro Dalben) for initial open-source publication; recommended but not mandatory; residual risk accepted. Preliminary screen 2026-08-19 found no obvious conflict; NOT LEGAL CLEARANCE. See docs/foundation.md and TRADEMARKS.md.
...
- Publication also remains subject to the separate identity and qualified legal-review recommendation (non-blocking by maintainer decision 2026-08-19; residual risk accepted); passing this firewall alone would not grant publication authority — remaining mandatory gates (final version/revision, generated gem provenance, rehearsal, configuration) still apply.
