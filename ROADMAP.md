...
- [x] **1.8 Agent Workflow & Review Orchestration** (2026-09-06) — ADR 0021, ReviewPacket/Observation/WorkflowReceipt, `review show|observe|complete`, `repair verify`, MCP 16 tools, BYOA, experiment 0/10 vs 6/10. Tag v1.8.0, GitHub Release; patch v1.8.1 (lib mapping arity), v1.8.2 (unmapped focus pointer, G3 MUST). RubyGems publication by maintainer decision. — ADR 0021, `ReviewPacket`/`ReviewObservation`/`WorkflowReceipt`, `review show|observe|complete`, `repair verify`, MCP +3 tools (16), BYOA flows, exploratory agent experiment. Branch `feat/1.8-agent-workflow`.
