...
- [x] **1.4 Agent Handoff & Evidence Reuse** (2026-08-26) — Handoff v1 `256 KiB` `handoff_id`, `Reuse.evaluate` single authority, `check --handoff` + `handoff create|inspect|verify` + MCP `create/inspect/verify_handoff` parity, `TOCTOU` guard.
- [x] **1.5 Rails-Native Adoption & CI Efficiency** (2026-08-27) — Native Brakeman analyzer, targeted test verification (`TestSelection` & `Resolvers::TestCandidates`), safe infrastructure fallbacks, per-analyzer tiered evidence reuse (`VerificationPlan`), finding ID normalization, and IntegrarPlus dogfooding (99.4% test savings). Tag v1.5.0, Lab 5.0 (136 PASS).
